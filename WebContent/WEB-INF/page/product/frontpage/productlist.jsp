...
<head><title>${producttype.name} 巴巴运动网</title>    
	<link href="<%=basePath%>/css/global/header01.css" rel="stylesheet" type="text/css">
	<link href="<%=basePath%>/css/product/list.css" rel="stylesheet" type="text/css" />	
	<link href="<%=basePath%>/css/global/topsell.css" rel="stylesheet" type="text/css">
...
<body class="ProducTypeHome2" onload="javascript:pageInit()">
    ...
    <div class="browse_right">
         <div class="select_reorder">
              ...
              <a title='按销量降序' href="<html:rewrite action="/product/list"/>?sort=selldesc&style=${param.style}&typeid=${param.typeid}">销量多到少</a></c:if>
			  | <c:if test="${'sellpricedesc'==param.sort}"><strong><em>价格高到低</em></strong></c:if><c:if test="${'sellpricedesc'!=param.sort}">
			  <a title='价格高到低' href="<html:rewrite action="/product/list"/>?sort=sellpricedesc&style=${param.style}&typeid=${param.typeid}">价格高到低</a></c:if>
			  | <c:if test="${'sellpriceasc'==param.sort}"><strong><em>价格低到低</em></strong></c:if><c:if test="${'sellpriceasc'!=param.sort}">
			  <a title='价格低到高' href="<html:rewrite action="/product/list"/>?sort=sellpriceasc&style=${param.style}&typeid=${param.typeid}&style=${param.style}">价格低到高</a></c:if>
			  | <c:if test="${empty param.sort}"><strong><em>最近上架时间</em></strong></c:if><c:if test="${!empty param.sort}">
			  <a title='价格低到高' href="<html:rewrite action="/product/list"/>?typeid=${param.typeid}">最近上架时间</a></c:if> </div>
			...
			<div class="emptybox"></div>
			 <div class="brand">
				...
				<li><a href="<html:rewrite action="/product/list"/>?sort=${param.sort}&style=${param.style}&typeid=${param.typeid}&brandid=${brand.code}&sex=${param.sex}">${brand.name}</a></li></c:forEach>
				</ul>
			 </div>
			 <div class="SubCategoryBox">
				<div class="FindByHint">按<strong>男女款</strong>选择：</div>
				<ul class="CategoryListTableLevel1">
				<li><a  href="<html:rewrite action="/product/list"/>?sort=${param.sort}&style=${param.style}&typeid=${param.typeid}&sex=MAN&brandid=${param.brandid}">男款</a></li>
				<li><a  href="<html:rewrite action="/product/list"/>?sort=${param.sort}&style=${param.style}&typeid=${param.typeid}&sex=WOMEN&brandid=${param.brandid}">女款</a></li>
				<li><a  href="<html:rewrite action="/product/list"/>?sort=${param.sort}&style=${param.style}&typeid=${param.typeid}&sex=NONE&brandid=${param.brandid}">男女均可</a></li>
				<li><a class="red" href="<html:rewrite action="/product/list"/>?sort=${param.sort}&style=${param.style}&typeid=${param.typeid}&brandid=${param.brandid}">全部</a></li>
				</ul>
			 </div>
		</div>
...
<c:forEach items="${pageView.records}" var="entry">	
        <div class="detail">
           <div class="goods" style="cursor:hand;background:url(<c:forEach items="${entry.styles}" var="pic"><%=basePath%>${pic.imageFullPath140}</c:forEach>) center center no-repeat"><a href="<html:rewrite action="/product/view"/>?productid=${entry.id}" target="_blank">
            <img src="<%=basePath%>/images/global/product_blank.gif" alt="${entry.name}" width="140" height="168"  border="0"/></a></div>
           <h2><a href="<html:rewrite action="/product/view"/>?productid=${entry.id}" target="_blank" title="${entry.name}">${entry.name}${entry.sellcount}</a></h2>
           <div class="save_number"><s>￥${entry.marketprice}</s>　<strong><em>￥${entry.sellprice}</em></strong>　节省：${entry.savedPrice}</div>
           <div class="an_img" align="center"><a href="<html:rewrite action="/product/view"/>?productid=${entry.id}"><img src='<%=basePath%>/images/sale.gif' width='84' height='24' border='0' class='a_1' /></a></div>
        </div>
</c:forEach>
<!----------------------LOOP END------------------------------->
		<div class='emptybox'></div>
