...
<link href="<%=basePath%>/css/product/product.css" rel="stylesheet" type="text/css">
<link href="<%=basePath%>/css/global/topcommend.css" rel="stylesheet" type="text/css">
...
<div id="ContentBody"><!-- 页面主体 -->
    <c:set var="out" value="&gt;&gt; <em>${product.name}</em>"/><c:forEach items="${types}" var="type" varStatus="statu">
		<c:set var="out" value=" &gt;&gt; <a href='/product/list.do?typeid=${type.typeid}'>${type.name}</a> ${out}"/>
	</c:forEach>
	 	 <div id="position"> 您现在的位置：<a href="<%=basePath%>/" name="linkHome">巴巴运动网</a> <span id="uc_cat_spnPath"><c:out value="${out}" escapeXml="false"></c:out></span></div>
 <div class="browse_left"><!-- 页面主体 左边 --> 
	...
	<DIV id="topcommend" align="left">
	       <DIV id="newtop"><IMG height=13 src="<%=basePath%>/images/global/sy2.gif" width=192></DIV>
	       <DIV id="newlist">
		  <DIV id="browse">
		    <DIV class="browse_t">精品推荐</DIV>
...
<div id="browse_left">
						...
						<c:forEach items="${product.styles}" var="style">
							<c:if test="${style.visible }"><c:set var="currentimage" value="${style }"/><c:set var="imagecount" value="${imagecount+1 }"/></c:if>
						</c:forEach>
						
							<div class="right_left">
								<div id="main_image" onclick="" style="cursor:hand;background:url('<%=basePath %>${currentimage.imageFullPath140}') center center no-repeat"><img src="<%=basePath%>/images/global/product_blank.gif" WIDTH="200" HEIGHT="240"/></div>
								<img src="" onclick="JavaScript:bigImageBrowse('${currentimage.id}')" style="cursor:hand;"/>
							</div>
							...
							<div class="right_right">									
									...
									<div class="right_desc">
											...
											<c:if test="${imagecount>1}">
												<img src="<%=basePath%>/images/global/init.gif" width="0" height="0">
												<li>颜色：<SELECT name="styleid" onchange="javascript:styleEvent(this.value)">
													<c:forEach items="${product.styles}" var="style"><c:if test="${style.visible}">
														<option value="${style.id }" <c:if test="${style.id==currentimage.id}">selected </c:if>>${style.name }</option></c:if>
													</c:forEach>
												</SELECT></li>
													<c:forEach items="${product.styles}" var="style"><c:if test="${style.visible}">
														<INPUT TYPE="hidden" id="productImage_${style.id }" value="<%=basePath %>${style.imageFullPath140}">
													    <INPUT TYPE="hidden" id="productPrototypeImage_${style.id }" value="<%=basePath %>${style.imageFullPath}"></c:if>
												    </c:forEach>
											</c:if>
									...
									<ul>
										<li>市场价：<s>${product.marketprice}</s> 元 <font color='#ff6f02'>本站价：<b>${product.sellprice} 元</b></font> 节省：<font color='#ff6f02'>${product.savedPrice }</font> 元										</li>
									  	<li class="right_img"><INPUT TYPE="image" src="<%=basePath%>/images/global/sale.gif"></li>
																				<li class="guopiprice">
										[ <IMG src="<%=basePath%>/images/global/2j4.gif" border="0">&nbsp;<A href="http://www.babasport.com/cache/news/6/9.shtml" target="_blank">配送说明</A> ]&nbsp;&nbsp;&nbsp;&nbsp;[ <IMG src="<%=basePath%>/images/global/2j4.gif" border="0">&nbsp;<A href="http://www.babasport.com/cache/news/4/24.shtml" target="_blank">付款方式</A> ]
										</li>
									</ul>									
							</div>
</div>
