...
<DIV id="sy_biankuang">
    ...
    <div class="browse_right">
         <div class="select_reorder">
              ...
              <a title='按销量降序' href="<html:rewrite action="/product/list"/>?sort=selldesc&typeid=${param.typeid}&style=${param.style}&sex=${param.sex }">销量多到少</a></c:if>
			  | <c:if test="${'sellpricedesc'==param.sort}"><strong><em>价格高到低</em></strong></c:if><c:if test="${'sellpricedesc'!=param.sort}">
			  <a title='价格高到低' href="<html:rewrite action="/product/list"/>?sort=sellpricedesc&typeid=${param.typeid}&style=${param.style}&sex=${param.sex }">价格高到低</a></c:if>
			  | <c:if test="${'sellpriceasc'==param.sort}"><strong><em>价格低到低</em></strong></c:if><c:if test="${'sellpriceasc'!=param.sort}">
			  <a title='价格低到高' href="<html:rewrite action="/product/list"/>?sort=sellpriceasc&typeid=${param.typeid}&style=${param.style}&sex=${param.sex }">价格低到高</a></c:if>
			  | <c:if test="${empty param.sort}"><strong><em>最近上架时间</em></strong></c:if><c:if test="${!empty param.sort}">
			  <a title='价格低到高' href="<html:rewrite action="/product/list"/>?sort=&typeid=${param.typeid}&style=${param.style}&sex=${param.sex }">最近上架时间</a></c:if> </div>
			...
			<div class="emptybox"></div>
			 ...
			 <div class="SubCategoryBox">
				...
				<li><a  href="<html:rewrite action="/product/list"/>?sort=${param.sort }&typeid=${param.typeid}&sex=MAN&brandid=${param.brandid}&style=${param.style}">男款</a></li>
				<li><a  href="<html:rewrite action="/product/list"/>?sort=${param.sort }&typeid=${param.typeid}&sex=WOMEN&brandid=${param.brandid}&style=${param.style}">女款</a></li>
				<li><a  href="<html:rewrite action="/product/list"/>?sort=${param.sort }&typeid=${param.typeid}&sex=NONE&brandid=${param.brandid}&style=${param.style}">男女均可</a></li>
				<li><a class="red" href="<html:rewrite action="/product/list"/>?sort=${param.sort }&typeid=${param.typeid}&style=${param.style}">全部</a></li>
				</ul>
			 </div>
		</div>
		...
		<div class="goodslist">
          ...
          <div class="goods_right">
			  ...
			  <c:if test="${!empty entry.brand}"> <li>品牌：${entry.brand.name}</li></c:if>
	           ...
	           <div class="content">产品说明：<c:out value="${fn:substring(entry.description,0,200)}"/></div>
	           <div class="message_bottom">
	                <div class="save"><s>￥${entry.marketprice}</s>　<strong><em>￥${entry.sellprice}</em></strong>　节省：${entry.savedPrice}</div>
			        <div class="buy"><a href="<html:rewrite action="/product/view"/>?productid=${entry.id}"><img src='<%=basePath %>/images/sale.gif' width='84' height='24' border='0' /></a></div>
	           </div>
          </div>
